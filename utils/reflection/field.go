...
var (
	...
	ptrTimeType    = reflect.TypeOf(&now)
)
...
func (f *Field) IsTime() bool {
	return f.value.Type().AssignableTo(timeType) || f.value.Type().AssignableTo(ptrTimeType)
}
...
func (f *Field) Kind() reflect.Kind {
	if f.value.Kind() == reflect.Ptr && !f.value.IsNil() {
		return f.value.Elem().Kind()
	}
	return reflect.ValueOf(f.Value()).Kind()
}
